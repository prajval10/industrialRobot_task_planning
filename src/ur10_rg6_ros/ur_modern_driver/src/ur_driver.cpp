...
bool UrDriver::rg6Callback(ur_control::RG6::Request &req, ur_control::RG6::Response &res)
{
	std::cout << "Reached service to contol RG6 gripper" << std::endl;

	rg6Control(req.target_width.data, 0, req.payload.data, false, false, false);
	return true;
}

void UrDriver::rg6Control(float target_width, int target_force, float payload, bool set_payload, bool depth_compensation, bool slave)
{
	...
	sprintf(buf1, "\tpayload=%f\n",payload);
	std::cout << "Reached service to control RG6 gripper" << std::endl;
	...
	cmd_str += "\ttarget_width=target_width+6.0\n";
...
void UrDriver::rg6GripDetect()
{
	std::string cmd_str;
	char buf_socket[5000];
	char buf_socket1[5000];

	cmd_str = "def rg6GripDetect():\n";
	...
	cmd_str += "\tzangle = zscale*1.57079633-0.0942477796\n";
	cmd_str += "\tzwidth = 8.4+160*sin(zangle)\n";
	cmd_str += "\tglobal measure_width = (floor(zwidth*10))/10-7.0\n";
	cmd_str += "\ttextmsg(\"width\",measure_width)\n";
	// sprintf(buf_socket, "\tsocket_open(\"%s\", %i)\n", ip_addr_.c_str(),50002);
	sprintf(buf_socket1, "\tsocket_open(\"%s\", %i)\n", ip_addr_.c_str(),50003);
	// cmd_str += buf_socket;
...
bool UrDriver::IKCallback(ur_modern_driver::IK::Request &req, ur_modern_driver::IK::Response &res)
{
	std::cout << "Reached IK service to go to object pose" << std::endl;

	getObjectPose(req.x.data, req.y.data);
	return true;
...
void UrDriver::getObjectPose(float x, float y){
	//Convert to meters.

	float x_off = (x/1000);
	float y_off = ((y+30)/1000);
	...
	char buf1[5000];
	char buf2[5000];
	cmd_str = "def getObjectPose():\n";
	sprintf(buf1, "\tglobal offset=p[%f,%f,0,0,0,0]\n",x_off,y_off);
	cmd_str += buf1;
	cmd_str += "\tglobal Origin_pose=p[1.05720, 0.17802, -0.09310, 3.1709, 0.4020, 0.0976]\n";
	cmd_str += "\tglobal target=pose_trans(Origin_pose,offset)\n";
	cmd_str += "\tmovel(pose_trans(p[0.0,0.0,0.07,0.0,0.0,0.0], target), a=0.5, v=0.25)\n";
	cmd_str += "\tmovel(pose_trans(p[0.0,0.0,0.001,0.0,0.0,0.0], target), a=0.5, v=0.25)\n";
	cmd_str += "end\n";
	rt_interface_->addCommandToQueue(cmd_str);
	std::cout << cmd_str << std::endl;	
}
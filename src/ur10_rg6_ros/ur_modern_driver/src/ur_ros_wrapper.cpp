...
protected:
    ...
    /// RG2 gripper service
    ros::ServiceServer rg6_service,rg6_detect_service;
...
public:
    ...
    RosWrapper(std::string host, int reverse_port) :
        ...
        // RG2 gripper
        rg6_service = nh_.advertiseService<ur_control::RG6::Request, ur_control::RG6::Response>
                ("/rg6_gripper/control_width", boost::bind(&UrDriver::rg6Callback, &robot_, _1, _2));
        // RG2 Grip_Detect
        rg6_detect_service = nh_.advertiseService<ur_control::RG6_Grip::Request, ur_control::RG6_Grip::Response>
                ("/rg6_gripper/grip_detect", boost::bind(&RosWrapper::rg6GripDetectServer, this, _1, _2));

		//IK service
        IK_service = nh_.advertiseService<ur_modern_driver::IK::Request, ur_modern_driver::IK::Response>
                ("/ur_IK_service", boost::bind(&UrDriver::IKCallback, &robot_, _1, _2));
...
private:
    ...
    void rg6GripDetectcallback(const std_msgs::Bool::ConstPtr& msg, bool &got_data)
    {
        if(msg->data)
        {
    ...
    bool rg6GripDetectServer(ur_control::RG6_Grip::Request &req, ur_control::RG6_Grip::Response &res)
    {
        bool flag = false;
        ros::Subscriber sub = nh_.subscribe<std_msgs::Bool>("/rg6_gripdetect", 1, boost::bind(&RosWrapper::rg6GripDetectcallback, this, _1,boost::ref(flag)));
        ros::Duration(0.7).sleep();
        robot_.rg6GripDetect();
        while(!flag);
        if(RosWrapper::response)
            res.gripped.data = true;
        else
            res.gripped.data = false;
